...
| Case | Command / Test | Expected result |
| --- | --- | --- |
| Industrial service contract | `python3 -m pytest -q -s tests/test_jellyfish_industrial_film_core.py` | Overview maps Jellyfish state into all 11 pipeline stages, exposes 9/9 starter-kit implementation phases, creation-entry semantics, and shooting gate; plan exposes render/QA/retry/post contracts only after the gate is ready; CineForge workflow state exposes persisted edit/regenerate/automation contracts. |
...
- `拍摄前置门禁` blocks render queues when novel/script text, shot graph,
  characters, actor identity references, scenes, props, costumes, shot details,
  or ready shots are missing
- Clicking `创建生产任务` writes industrial task records into the Jellyfish task
  center only after the gate is ready; blocked projects must resolve the gate
  before render tasks are exposed
...
- Jellyfish now has native Film Core overview, text-to-drama intake, generated
  novel/assets/storyboard persistence, shooting gate, plan preview, and task
  writeback endpoints plus persisted workflow-state edit/regenerate/complete
